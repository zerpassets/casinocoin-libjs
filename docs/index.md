...
- [casinocoind APIs](#casinocoind-apis)
...
- [API Methods](#api-methods)
  ...
  - [cscToDrops](#csctodrops)
  - [dropsToCsc](#dropstocsc)
  - [iso8601ToCasinocoinTime](#iso8601tocasinocointime)
  - [txFlags](#txflags)
...
CasinocoinAPI (casinocoin-libjs) is the official client library to the CSC Ledger. Currently, CasinocoinAPI is only available in JavaScript.
...
maxFeeCSC | string | *Optional* Maximum fee to use with transactions, in CSC. Must be a string-encoded number. Defaults to `'2'`.
...
server | uri string | *Optional* URI for casinocoind websocket port to connect to. Must start with `wss://`, `ws://`, `wss+unix://`, or `ws+unix://`.
...
Name | Type | Description
---- | ---- | -----------
currency | [currency](#currency) | The three-character code or hexadecimal string used to denote currencies, or "drops" for the smallest unit of CSC.
counterparty | [address](#casinocoin-address) | *Optional* The Casinocoin address of the account that owes or is owed the funds (omitted if `currency` is "CSC" or "drops")
...
[paymentChannelCreate](#payment-channel-create) | A `paymentChannelCreate` transaction opens a payment channel between two addresses with CSC set aside for asynchronous payments.
[paymentChannelFund](#payment-channel-fund) | A `paymentChannelFund` transaction adds CSC to a payment channel and optionally sets a new expiration for the channel.
[paymentChannelClaim](#payment-channel-claim) | A `paymentChannelClaim` transaction withdraws CSC from a channel and optionally requests to close it.
...
maxFee | [value](#value) | *Optional* Deprecated: Use `maxFeeCSC` in the CasinocoinAPI constructor instead. The maximum fee to pay for this transaction. If this exceeds `maxFeeCSC`, `maxFeeCSC` will be used instead. See [Transaction Fees](#transaction-fees) for more information.
...
*source.* address | [address](#casinocoin-address) | The address to send from.
...
*destination.* address | [address](#casinocoin-address) | The address to receive at.
...
*destination.* address | [address](#casinocoin-address) | The address to send to.
...
noDirectCasinocoin | boolean | *Optional* If true and paths are specified, the sender would like the CSC Ledger to disregard any direct paths from the source account to the destination account. This may be used to take advantage of an arbitrage opportunity or by gateways wishing to issue balances from a hot wallet to a user who has mistakenly set a trustline directly to the hot wallet.
...
{
  "source": {
    "address": "cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb",
    "maxAmount": {
      "value": "0.01",
      "currency": "USD",
      "counterparty": "cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne"
  ...
  "destination": {
    "address": "chnD92a3R6Eyr2x7ujNmS7GnwFDf3FrFbr",
    "amount": {
      "value": "0.01",
      "currency": "USD",
      "counterparty": "cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne"
...
counterparty | [address](#casinocoin-address) | The address of the account this trustline extends trust to.
...
Name | Type | Description
---- | ---- | -----------
defaultCasinocoin | boolean | *Optional* Enable [rippling](https://casinocoin.org/build/understanding-the-noripple-flag/) on this account’s trust lines by default. (New in [casinocoind 0.27.3](https://github.com/casinocoin/casinocoind/releases/tag/0.27.3))
depositAuth | boolean | *Optional* Enable [Deposit Authorization](https://casinocoin.org/build/deposit-authorization/) on this account. If set, transactions cannot send value of any kind to this account unless the sender of those transactions is the account itself. (Requires the [DepositAuth amendment](https://casinocoin.org/build/known-amendments/#depositauth))
...
regularKey | [address](#casinocoin-address),null | *Optional* The public key of a new keypair, to use as the regular key to this account, as a base-58-encoded string in the same format as an account address. Use `null` to remove the regular key.
...
*signers.weights[].* address | [address](#casinocoin-address) | A Casinocoin account address
...
destination | [address](#casinocoin-address) | Address to receive escrowed CSC.
...
```json
{
  "destination": "chnD92a3R6Eyr2x7ujNmS7GnwFDf3FrFbr",
  "amount": "0.01",
...
Name | Type | Description
---- | ---- | -----------
owner | [address](#casinocoin-address) | The address of the owner of the escrow to cancel.
...
```json
{
  "owner": "cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb",
  "escrowSequence": 1234
}
```
...
Name | Type | Description
---- | ---- | -----------
owner | [address](#casinocoin-address) | The address of the owner of the escrow to execute.
...
Name | Type | Description
---- | ---- | -----------
destination | [address](#casinocoin-address) | Address of the account that can cash the check.
sendMax | [laxAmount](#amount) | Amount of source currency the check is allowed to debit the sender, including transfer fees on non-CSC currencies.
...
```json
{
  "destination": "cJcyvdykuUychWqNehMyN1BqbxF8xcfdhw",
  "sendMax": {
    "currency": "drops",
    "value": "100000000"
  }
...
destination | [address](#casinocoin-address) | Address to receive CSC claims against this channel.
...
---- | ---- | -----------
channel | string | 256-bit hexadecimal channel identifier.
amount | [value](#value) | *Optional* Amount of CSC authorized by this signature.
balance | [value](#value) | *Optional* Total CSC balance delivered by this channel after claim is processed.
close | boolean | *Optional* Request to close the channel. If the channel has no CSC remaining or the destination address requests it, closes the channel immediately (returning unclaimed CSC to the source address). Otherwise, sets the channel to expire after settleDelay seconds have passed.
...
signature | string | *Optional* Signed claim authorizing withdrawal of CSC from the channel. (Required except from the channel's source address.)
...
Returns the response from invoking the specified command, with the specified options, on the connected casinocoind server.

Refer to [casinocoind APIs](https://casinocoin.org/build/casinocoind-apis/) for commands and options. All CSC amounts must be specified in drops. One drop is equal to 0.00000001 CSC. See [Specifying Currency Amounts](https://casinocoin.org/build/casinocoind-apis/#specifying-currency-amounts).

Most commands return data for the `current` (in-progress, open) ledger by default. Do not rely on this. Always specify a ledger version in your request. In the example below, the 'validated' ledger is requested, which is the most recent ledger that has been validated by the whole network. See [Specifying Ledgers](https://casinocoin.org/build/casinocoind-apis/#specifying-ledgers).

### Return Value

This method returns a promise that resolves with the response from casinocoind.

### Example

```javascript
// Replace 'ledger' with your desired casinocoind command
...
See [Markers and Pagination](https://casinocoin.org/build/casinocoind-apis/#markers-and-pagination).
...
This method returns a promise that resolves with the next page of data from casinocoind.
...
Returns an object with the `counterparty` field renamed to `issuer`. This is useful because CasinocoinAPI generally uses the name `counterparty` while the casinocoind API generally uses the name `issuer`.

This is a static method on the `CasinocoinAPI` class.
...
const orderbookInfo = {
  "base": {
    "currency": "USD",
    "counterparty": "cpa54XMDD4U9SNpx3xDQqDecUX9r6wcbKd"
  },
  "counter": {
    "currency": "BTC",
    "counterparty": "cpa54XMDD4U9SNpx3xDQqDecUX9r6wcbKd"
...
console.log(CasinocoinAPI.renameCounterpartyToIssuer(orderbookInfo.base))
console.log(CasinocoinAPI.renameCounterpartyToIssuer(orderbookInfo.counter))
```

```
{ currency: 'USD', issuer: 'cpa54XMDD4U9SNpx3xDQqDecUX9r6wcbKd' }
{ currency: 'BTC', issuer: 'cpa54XMDD4U9SNpx3xDQqDecUX9r6wcbKd' }
```
...
*bids[].properties.* maker | [address](#casinocoin-address) | The address of the account that submitted the order.
...
*asks[].properties.* maker | [address](#casinocoin-address) | The address of the account that submitted the order.
...
For details, see the casinocoind method [book_offers](https://casinocoin.org/build/casinocoind-apis/#book-offers).
...
return Promise.all(
  [
    this.api.request('book_offers', {
      taker_gets: CasinocoinAPI.renameCounterpartyToIssuer(orderbookInfo.base),
      taker_pays: CasinocoinAPI.renameCounterpartyToIssuer(orderbookInfo.counter),
      ledger_index: 'validated',
      limit: 20,
      taker: address
    }),
    this.api.request('book_offers', {
      taker_gets: CasinocoinAPI.renameCounterpartyToIssuer(orderbookInfo.counter),
      taker_pays: CasinocoinAPI.renameCounterpartyToIssuer(orderbookInfo.base),
      ledger_index: 'validated',
      limit: 20,
      taker: address
    })
  ...
  const orderbook = CasinocoinAPI.formatBidsAndAsks(orderbookInfo, [...directOffers, ...reverseOffers]);
  ...
  "bids": [
    {
      ...
      "properties": {
        "maker": "cNPx4aPFtGawTdiJ39jEvrRAvh1kTYAeHR",
        "sequence": 262660,
        "makerExchangeRate": "4297.264683713081"
      },
      "data": {
        "Account": "cNPx4aPFtGawTdiJ39jEvrRAvh1kTYAeHR",
    ...
    },
    {
      ...
      "properties": {
        "maker": "cp18BNtG96vCotjLJMyFoLBtb7s25HtfCm",
        "sequence": 231459,
        "makerExchangeRate": "4335.663628792661"
      },
      "data": {
        "Account": "cp18BNtG96vCotjLJMyFoLBtb7s25HtfCm",
...
This will use the [feeCushion parameter](#parameters) provided to the CasinocoinAPI constructor, or the default value of `1.2`.

### Parameters

Name | Type | Description
---- | ---- | -----------
cushion | number | *Optional* The fee is the product of the base fee, the `load_factor`, and this cushion. Default is provided by the `CasinocoinAPI` constructor's `feeCushion`.
...
address | [address](#casinocoin-address) | The address of the account that initiated the transaction.
...
*outcome.balanceChanges.* \* | array\<[balance](#amount)\> | Key is the CSC Ledger address; value is an array of signed amounts representing changes of balances for that address.
*outcome.orderbookChanges.* \* | array | Key is the maker's CSC Ledger address; value is an array of changes
...
```json
{
  ...
  "specification": {
    ...
    "destination": {
      "address": "cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne"
    },
    "paths": "[[{\"currency\":\"USD\",\"issuer\":\"chnD92a3R6Eyr2x7ujNmS7GnwFDf3FrFbr\",\"type\":48,\"type_hex\":\"0000000000000030\"},{\"account\":\"chnD92a3R6Eyr2x7ujNmS7GnwFDf3FrFbr\",\"currency\":\"USD\",\"issuer\":\"chnD92a3R6Eyr2x7ujNmS7GnwFDf3FrFbr\",\"type\":49,\"type_hex\":\"0000000000000031\"}]]"
  },
  "outcome": {
    "result": "tesSUCCESS",
    ...
    "balanceChanges": {
      "chnD92a3R6Eyr2x7ujNmS7GnwFDf3FrFbr": [
        {
          "counterparty": "cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne",
          "currency": "USD",
          "value": "-0.001"
        ...
        {
          "counterparty": "cK2h6nq7ymW1Lqiqu9AYYTUVLTrUbmSqsz",
          "currency": "USD",
          "value": "0.001002"
        }
      ],
      "cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne": [
        {
          "counterparty": "chnD92a3R6Eyr2x7ujNmS7GnwFDf3FrFbr",
          "currency": "USD",
          "value": "0.001"
        }
      ],
      ...
      "cK2h6nq7ymW1Lqiqu9AYYTUVLTrUbmSqsz": [
        {
          "currency": "CSC",
          "value": "0.01101198"
        },
...
Name | Type | Description
---- | ---- | -----------
address | [address](#casinocoin-address) | The address of the account to get transactions for.
...
*options.* counterparty | [address](#casinocoin-address) | *Optional* If provided, only return transactions with this account as a counterparty to the transaction.
...
[
  {
    ...
    "address": "cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb",
    "sequence": 4,
    "id": "167E2205FE17AAA98AEE6815547118CB02A0157C054BCCB5EE1CD98C7780958F",
    "specification": {
      "memos": [
        {
          "type": "client",
      ...
      "paths": "[[{\"issuer\":\"chnD92a3R6Eyr2x7ujNmS7GnwFDf3FrFbr\",\"currency\":\"USD\"},{\"account\":\"chnD92a3R6Eyr2x7ujNmS7GnwFDf3FrFbr\",\"issuer\":\"chnD92a3R6Eyr2x7ujNmS7GnwFDf3FrFbr\",\"currency\":\"USD\"}]]"
    },
    "outcome": {
      "result": "tesSUCCESS",
      "fee": "0.0000001",
...
Name | Type | Description
---- | ---- | -----------
address | [address](#casinocoin-address) | The address of the account to get trustlines for.
options | object | *Optional* Options to filter and determine which trustlines to return.
*options.* counterparty | [address](#casinocoin-address) | *Optional* Only return trustlines with this counterparty.
...
[
  {
    "specification": {
      "limit": "5",
      "currency": "USD",
      "counterparty": "chvDYvKFV7iNXezNLTrMfM5Dre1Hu4vpZQ",
      "ripplingDisabled": true,
      "frozen": true
    },
    "counterparty": {
  ...
  },
  {
    "specification": {
      "limit": "1",
      "currency": "USD",
      "counterparty": "c5bvVgDDE8uG4yiotzqPdDaUCHSNMQxTy"
    },
    "counterparty": {
      "limit": "0"
    },
  ...
  },
  {
    "specification": {
      "limit": "1",
      "currency": "USD",
      "counterparty": "cN9vMjt3WUSqFp3DSvHugSivyG3WQk1XnP",
      "ripplingDisabled": true
    },
    "counterparty": {
      "limit": "0"
  ...
  },
  {
    "specification": {
      "limit": "500",
      "currency": "USD",
      "counterparty": "cLgUQNXJREwKGKy5k4umSdHqv3xEb8RPre",
      "ripplingDisabled": true
    },
    "counterparty": {
      "limit": "0"
  ...
  },
  {
    "specification": {
      "limit": "0",
      "currency": "USD",
      "counterparty": "cD8tB8pVG6vx3hhJ2YQqpqMA1gWETYxgij"
    },
    "counterparty": {
      "limit": "100",
      "ripplingDisabled": true
  ...
  },
  {
    "specification": {
      "limit": "0",
      "currency": "USD",
      "counterparty": "css6ZY7UxR3VR62mVCPmsDwYw4RkmV3ZYj",
      "frozen": true
    },
    "counterparty": {
      "limit": "1"
...
Name | Type | Description
---- | ---- | -----------
address | [address](#casinocoin-address) | The address of the account to get balances for.
options | object | *Optional* Options to filter and determine which balances to return.
*options.* counterparty | [address](#casinocoin-address) | *Optional* Only return balances with this counterparty.
...
counterparty | [address](#casinocoin-address) | *Optional* The Casinocoin address of the account that owes or is owed the funds.
...
[
  ...
  },
  {
    "value": "0",
    "currency": "ASP",
    "counterparty": "c9EpBpA2DpxK7ik2QZMXCnSmTEWPBhBB7n"
  },
  {
    "value": "0",
    "currency": "XAU",
  ...
  },
  {
    ...
    "currency": "MXN",
    "counterparty": "cGQjiWgoNq9AKtADHYPrthFnaAGT8mkr8p"
  },
  {
  ...
  },
  {
    "value": "0",
    "currency": "CNY",
    "counterparty": "cGt4mcHK7VCyEELkJDJUDuz4qaw4n41wur"
  },
  {
    "value": "1.294889190631542",
    "currency": "DYM",
    "counterparty": "cnknXuFpi8ZUAVr5z5WDTpos2J9WPdH456"
  },
  {
  ...
  },
  {
    "value": "-0.00111",
    "currency": "BTC",
    "counterparty": "c4U5pRntJ1EKbNntjDkK6GLPmP8jUKEEBY"
  },
  {
    "value": "-0.1010780000080207",
    "currency": "BTC",
    "counterparty": "caKvaQQRwHmG4tobxjuWWyLLUcTuhhvYXL"
  },
  {
    "value": "1",
    "currency": "USD",
  ...
  },
  {
    ...
    "currency": "CNY",
    "counterparty": "csNdreBaviA4oetFcJaLd7sqaXXSq6b346"
  },
  {
  ...
  },
  {
    "value": "-5",
    "currency": "JOE",
    "counterparty": "cP21f2sRDBN68d1gi6XKGRr98R7HEFyWqW"
  },
  {
    "value": "0",
    "currency": "USD",
  ...
  },
  {
    ...
    "counterparty": "cU2iMovvf9mHJCPeC3i8btC4x2YpGE6gUY"
  },
  {
    "value": "0",
    "currency": "USD",
...
Name | Type | Description
---- | ---- | -----------
address | [address](#casinocoin-address) | The Casinocoin address of the account to get the balance sheet of.
options | object | *Optional* Options to determine how the balances will be calculated.
*options.* excludeAddresses | array\<[address](#casinocoin-address)\> | *Optional* Addresses to exclude from the balance totals.
...
```json
{
  "balances": [
    {
    "counterparty": "cNqxmC6cmmaEBVeBwZ1WxYy7BioQfyBLrm",
    "currency": "EUR",
  ...
  {
    "counterparty": "cYhGbm8oq8L1U8JyFUm5eVa9LCiZsAtRK",
    "currency": "USD",
    "value": "13857.70416"
  }
  ],
  "assets": [
    {
    "counterparty": "cGw6JHuiyQvsK9QMefvHuWt3zt9F7qk3G5",
    "currency": "BTC",
  ...
  {
    "counterparty": "c9eoy6wQ1A7zeBUXgbMrtJkNSCgkAYVN93",
    "currency": "BTC",
...
*pathfind.source.* address | [address](#casinocoin-address) | The Casinocoin address of the account where funds will come from.
...
*pathfind.source.currencies[].* counterparty | [address](#casinocoin-address) | *Optional* The counterparty for the currency; if omitted any counterparty may be used.
*pathfind.* destination | object | Properties of the destination of funds.
*pathfind.destination.* address | [address](#casinocoin-address) | The address to send to.
...
[
  {
    ...
    "paths": "[[{\"account\":\"cHtfMUQbfbThu26c3kppTTAgVFj9fgicWD\"},{\"currency\":\"USD\",\"issuer\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"},{\"account\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"}],[{\"account\":\"cHtfMUQbfbThu26c3kppTTAgVFj9fgicWD\"},{\"currency\":\"CSC\"},{\"currency\":\"USD\",\"issuer\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"},{\"account\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"}],[{\"account\":\"cHtfMUQbfbThu26c3kppTTAgVFj9fgicWD\"},{\"currency\":\"CSC\"},{\"currency\":\"USD\",\"issuer\":\"cnqge9be8LoWCC7vm5SFZCJKcWBRSUdi1F\"},{\"account\":\"cnqge9be8LoWCC7vm5SFZCJKcWBRSUdi1F\"},{\"account\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"}],[{\"account\":\"cHtfMUQbfbThu26c3kppTTAgVFj9fgicWD\"},{\"currency\":\"CSC\"},{\"currency\":\"USD\",\"issuer\":\"cUtRir8za9SmW7aHAXt4VmvVrH2sr4J6mr\"},{\"account\":\"cUtRir8za9SmW7aHAXt4VmvVrH2sr4J6mr\"},{\"account\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"}]]"
  },
  {
    "source": {
      "address": "cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb",
    ...
    "paths": "[[{\"account\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"}],[{\"account\":\"chvDYvKFV7iNXezNLTrMfM5Dre1Hu4vpZQ\"},{\"currency\":\"USD\",\"issuer\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"},{\"account\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"}],[{\"account\":\"chvDYvKFV7iNXezNLTrMfM5Dre1Hu4vpZQ\"},{\"currency\":\"CSC\"},{\"currency\":\"USD\",\"issuer\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"},{\"account\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"}],[{\"account\":\"chvDYvKFV7iNXezNLTrMfM5Dre1Hu4vpZQ\"},{\"currency\":\"CSC\"},{\"currency\":\"USD\",\"issuer\":\"cnqge9be8LoWCC7vm5SFZCJKcWBRSUdi1F\"},{\"account\":\"cnqge9be8LoWCC7vm5SFZCJKcWBRSUdi1F\"},{\"account\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"}]]"
  },
  {
    "source": {
      "address": "cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb",
    ...
    "paths": "[[{\"currency\":\"USD\",\"issuer\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"},{\"account\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"}],[{\"currency\":\"USD\",\"issuer\":\"cgR4iCd1mm7drar6Ue3ay5BJQojBEcagG\"},{\"account\":\"cgR4iCd1mm7drar6Ue3ay5BJQojBEcagG\"},{\"account\":\"cDpMiqENzuEBx3MhxHn2sVemTCKg6QMRgc\"},{\"account\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"}],[{\"currency\":\"USD\",\"issuer\":\"cNb6keCEeQkuZFWBcHqqtvijpSpGSKCn1v\"},{\"account\":\"cNb6keCEeQkuZFWBcHqqtvijpSpGSKCn1v\"},{\"account\":\"cQDnRfCiy4c8ACpKLmooUqpTANddcHBwYL\"},{\"account\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"}],[{\"currency\":\"USD\",\"issuer\":\"cnqge9be8LoWCC7vm5SFZCJKcWBRSUdi1F\"},{\"account\":\"cnqge9be8LoWCC7vm5SFZCJKcWBRSUdi1F\"},{\"account\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"}]]"
  }
]
```
...
Name | Type | Description
---- | ---- | -----------
address | [address](#casinocoin-address) | The Casinocoin address of the account to get open orders for.
...
*properties.* maker | [address](#casinocoin-address) | The address of the account that submitted the order.
...
[
  {
    ...
    "properties": {
      "maker": "cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb",
      "sequence": 719930,
  ...
  },
  {
    "specification": {
      ...
      "quantity": {
        ...
        "counterparty": "cHLCLkBJyuZBuHeRfTy6Byn2SbHx1S1udj"
      },
      "totalPrice": {
        "currency": "USD",
  ...
  },
  {
    "specification": {
      ...
      "totalPrice": {
        ...
        "counterparty": "cgR4iCd1mm7drar6Ue3ay5BJQojBEcagG"
      }
    },
    "properties": {
      "maker": "cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb",
  ...
  },
  {
    "specification": {
      ...
      "quantity": {
        ...
        "counterparty": "c4g1fP1pqmVsoSi7X7MMA9xhQUXxuSazvm"
      },
      "totalPrice": {
        "currency": "USD",
  ...
  },
  {
    "specification": {
      ...
      "quantity": {
        ...
        "counterparty": "cKQfTmebA8EWmEWFvntykZfHB2gdQ245rZ"
      },
      "totalPrice": {
        "currency": "XAU",
  ...
  },
  {
    ...
    "properties": {
      "maker": "cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb",
      "sequence": 814018,
      "makerExchangeRate": "169.2262953364839"
    }
  },
  {
    "specification": {
  ...
  },
  {
    ...
    "properties": {
      "maker": "cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb",
      "sequence": 843730,
      "makerExchangeRate": "0.04485854972648762"
    }
  },
  {
    "specification": {
...
**Breaking change:** In casinocoin-libjs 1.1.0 and earlier, orders returned by this method were not sorted correctly. Orders are now sorted correctly, from best to worst.
...
Name | Type | Description
---- | ---- | -----------
address | [address](#casinocoin-address) | Address of an account to use as point-of-view. (This affects which unfunded offers are returned.)
  ...
  "bids": [
    {
      ...
      "properties": {
        "maker": "cDNYBTTga764ZEAux4knBLmizf2sYvEJWK",
        "sequence": 386940,
        "makerExchangeRate": "326.5003614141928"
      },
      "data": {
        "Account": "cDNYBTTga764ZEAux4knBLmizf2sYvEJWK",
    ...
    },
    {
      ...
      "properties": {
        "maker": "ch8j96rkcZfegwdGoVptHLakDysfmZ8AmN",
        "sequence": 207855,
        "makerExchangeRate": "330.6364334177034"
      },
      "data": {
        "Account": "ch8j96rkcZfegwdGoVptHLakDysfmZ8AmN",
    ...
    },
    {
      ...
      "properties": {
        "maker": "cQNBbwyjkUiePiHpT7zdAmM9DTMUfzFZCt",
        "sequence": 110103,
        "makerExchangeRate": "331.1338298016111"
      },
      "data": {
        "Account": "cQNBbwyjkUiePiHpT7zdAmM9DTMUfzFZCt",
    ...
    },
    {
      ...
      "properties": {
        "maker": "c45xCnvdofpXid7FCAnwaXPj9Gkgq1kdva",
        "sequence": 392,
      ...
      "data": {
        "Account": "c45xCnvdofpXid7FCAnwaXPj9Gkgq1kdva",
    ...
    },
    {
      ...
      "properties": {
        "maker": "cfi2Sod46mmEsPecytk7D7FDNt6uAnspFu",
        "sequence": 110061,
        "makerExchangeRate": "347.2306949944844"
      },
      "data": {
        "Account": "cfi2Sod46mmEsPecytk7D7FDNt6uAnspFu",
    ...
    },
    {
      ...
      "properties": {
        "maker": "cM3b2kvr2V26gYFZ8qCM527SaJY9h668qf",
        "sequence": 35788,
        "makerExchangeRate": "352.7092203179974"
      },
      "data": {
        "Account": "cM3b2kvr2V26gYFZ8qCM527SaJY9h668qf",
    ...
    },
    {
      ...
      "properties": {
        "maker": "cGXtZ181hLcpj2JsdsE86fjckZQHA5Gyp",
        "sequence": 491,
      ...
      "data": {
        "Account": "cGXtZ181hLcpj2JsdsE86fjckZQHA5Gyp",
    ...
    },
    {
      ...
      "properties": {
        "maker": "c3GqXFabuMxAcjs1tvWA7x2hyvMYFEi4wZ",
        "sequence": 5255,
      ...
      "data": {
        "Account": "c3GqXFabuMxAcjs1tvWA7x2hyvMYFEi4wZ",
  ...
  "asks": [
    {
      ...
      "properties": {
        "maker": "cUrCsPWGw2GebEMDusycxbKjRCXaruyhm4",
        "sequence": 434,
        "makerExchangeRate": "0.003120027456241615"
      },
      "data": {
        "Account": "cUrCsPWGw2GebEMDusycxbKjRCXaruyhm4",
    ...
    },
    {
      ...
      "properties": {
        "maker": "chpQmus693ctXxazYPZ2hxbGCyjsejGr1r",
        "sequence": 233,
        "makerExchangeRate": "0.003125"
      },
      "data": {
        "Account": "chpQmus693ctXxazYPZ2hxbGCyjsejGr1r",
...
Name | Type | Description
---- | ---- | -----------
address | [address](#casinocoin-address) | The address of the account to get the settings of.
...
```json
{
  ...
  "signers": {
    ...
    "weights": [
      {
        "address": "cU3Ve6174VEwEXFMLjuwipxy8VYGBiBgXy",
        "weight": 1
      }, {
        "address": "cD4SGmvMtF9Z2SxyGiSSJihSoMCUT6vUs3",
        "weight": 1
      }, {
        "address": "cBfY22ZsqPuKFPyA91q1g9mVEZ5tEUEabA",
        "weight": 1
      }
    ]
  }
...
Name | Type | Description
---- | ---- | -----------
address | [address](#casinocoin-address) | The address of the account to get the account info of.
options | object | *Optional* Options that affect what to return.
...
Name | Type | Description
---- | ---- | -----------
address | [address](#casinocoin-address) | The address of the account to get the account objects of.
options | object | *Optional* Options that affect what to return.
...
Name | Type | Description
---- | ---- | -----------
account | [address](#casinocoin-address) | Unique address of the account this request corresponds to.
...
* CasinocoinState objects for trust lines where this account's side is not in the default state.
  ...
  "account_objects": [
    {
      "Balance": {
        "currency": "ASP",
        "issuer": "cKVmFg7MnJepdx5N9HNZaqEF5rtYUQSiin",
        "value": "0"
      },
      "Flags": 65536,
      "HighLimit": {
        "currency": "ASP",
        "issuer": "cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb",
        "value": "0"
      },
      ...
      "LowLimit": {
        "currency": "ASP",
        "issuer": "c9EpBpA2DpxK7ik2QZMXCnSmTEWPBhBB7n",
        "value": "10"
      },
    ...
    },
    {
      ...
      "HighLimit": {
        "currency": "MXN",
        "issuer": "cGQjiWgoNq9AKtADHYPrthFnaAGT8mkr8p",
        "value": "0"
      },
      "HighNode": "0000000000000000",
      "LedgerEntryType": "CasinocoinState",
    ...
    },
    {
      ...
      "HighLimit": {
        "currency": "EUR",
        "issuer": "c5bvVgDDE8uG4yiotzqPdDaUCHSNMQxTy",
        "value": "0"
      },
      "HighNode": "0000000000000000",
      "LedgerEntryType": "CasinocoinState",
    ...
    },
    {
      ...
      "LowLimit": {
        "currency": "CNY",
        "issuer": "cGt4mcHK7VCyEELkJDJUDuz4qaw4n41wur",
        "value": "0"
      },
    ...
    },
    {
      ...
      "HighLimit": {
        "currency": "DYM",
        "issuer": "cnknXuFpi8ZUAVr5z5WDTpos2J9WPdH456",
        "value": "0"
      },
      "HighNode": "0000000000000000",
      "LedgerEntryType": "CasinocoinState",
...
Name | Type | Description
---- | ---- | -----------
account | [address](#casinocoin-address) | Address that created the payment channel.
destination | [address](#casinocoin-address) | Address to receive CSC claims against this channel.
...
```json
{
  "account": "c3JBi1HR75QBpFhYVU86KaMfvVEGr2HYSf",
  "amount": "0.1",
  "balance": "0",
  "destination": "cHVriY1YctocCawJCAzT6mMgt7X47hqMpY",
...
Parse an `AccountRoot` object's [`Flags`](https://casinocoin.org/build/reference-ledger-format.html#accountroot-flags).
...
This method returns an object with containing a key for each AccountRoot flag known to this version of CasinocoinAPI. Each flag has a boolean value of `true` or `false`.
...
```json
{
  ...
  "depositAuth": true,
  "disallowIncomingCSC": false,
  "disableMasterKey": false,
  "noFreeze": false,
  "globalFreeze": false,
  "defaultCasinocoin": false
}
```
...
This method works with any of [the transaction types supported by casinocoind](casinocoin.org/build/reference-transaction-format.html#transaction-types).

Notably, this is the preferred method for preparing a `DepositPreauth` transaction (added in casinocoind 1.1.0).

### Parameters

Name | Type | Description
---- | ---- | -----------
transaction | [transaction](https://casinocoin.org/build/reference-transaction-format.html#transaction-types) | The specification (JSON) of the transaction to prepare. Set `Account` to the address of the account that is creating the transaction. You may omit auto-fillable fields like `Fee`, `Flags`, and `Sequence` to have them set automatically.
...
async function preparedPreauth() {
  ...
  return api.prepareTransaction({
    ...
    Account: address,
    Authorize: 'cwzFxAgsVkgLuyHbrqdHXAJR5Do1Nbn75A'
  });
}
```
...
```javascript
{
  txJSON: '{"TransactionType":"DepositPreauth","Account":"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb","Authorize":"cwzFxAgsVkgLuyHbrqdHXAJR5Do1Nbn75A","Flags":2147483648,"LastLedgerSequence":13561714,"Fee":"12","Sequence":1}',
  instructions: {
    fee: '0.000012',
    sequence: 1,
...
Name | Type | Description
---- | ---- | -----------
address | [address](#casinocoin-address) | The address of the account that is creating the transaction.
...
```json
{
  "txJSON": "{\"Flags\":2147483648,\"TransactionType\":\"Payment\",\"Account\":\"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb\",\"Destination\":\"chnD92a3R6Eyr2x7ujNmS7GnwFDf3FrFbr\",\"Amount\":{\"value\":\"0.01\",\"currency\":\"USD\",\"issuer\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"},\"SendMax\":{\"value\":\"0.01\",\"currency\":\"USD\",\"issuer\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"},\"LastLedgerSequence\":8820051,\"Fee\":\"1200\",\"Sequence\":23}",
  "instructions": {
    "fee": "0.000012",
    "sequence": 23,
...
```json
{
  "txJSON": "{\"TransactionType\":\"TrustSet\",\"Account\":\"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb\",\"LimitAmount\":{\"currency\":\"USD\",\"issuer\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\",\"value\":\"10000\"},\"Flags\":2149711872,\"QualityIn\":910000000,\"QualityOut\":870000000,\"Memos\":[{\"Memo\":{\"MemoData\":\"7465787465642064617461\",\"MemoType\":\"74657374\",\"MemoFormat\":\"746578742F706C61696E\"}}],\"LastLedgerSequence\":8820051,\"Fee\":\"1200\",\"Sequence\":23}",
  "instructions": {
    "fee": "0.000012",
    "sequence": 23,
    "maxLedgerVersion": 8820051
...
// Buy 10.10 USD (of the specified issuer) for 2.0 CSC (2000000 drops), fill or kill.
const order = {
  "direction": "buy",
  "quantity": {
    "currency": "USD",
...
```json
{
  "txJSON": "{\"Flags\":2147745792,\"TransactionType\":\"OfferCreate\",\"Account\":\"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb\",\"TakerGets\":\"2000000\",\"TakerPays\":{\"value\":\"10.1\",\"currency\":\"USD\",\"issuer\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"},\"LastLedgerSequence\":8819954,\"Fee\":\"1200\",\"Sequence\":23}",
  "instructions": {
    "fee": "0.000012",
    "sequence": 23,
...
```json
{
  "txJSON": "{\"Flags\":2147483648,\"TransactionType\":\"OfferCancel\",\"Account\":\"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb\",\"OfferSequence\":23,\"LastLedgerSequence\":8820051,\"Fee\":\"1200\",\"Sequence\":23}",
  "instructions": {
    "fee": "0.000012",
    "sequence": 23,
    "maxLedgerVersion": 8820051
...
```json
{
  "txJSON": "{\"Flags\":2147483648,\"TransactionType\":\"EscrowCreate\",\"Account\":\"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb\",\"Destination\":\"chnD92a3R6Eyr2x7ujNmS7GnwFDf3FrFbr\",\"Amount\":\"1000000\",\"CancelAfter\":536544000,\"FinishAfter\":464908910,\"LastLedgerSequence\":8820051,\"Fee\":\"1200\",\"Sequence\":23}",
  "instructions": {
    "fee": "0.000012",
    "sequence": 23,
    "maxLedgerVersion": 8820051
...
```json
{
  "txJSON": "{\"Flags\":2147483648,\"TransactionType\":\"EscrowCancel\",\"Account\":\"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb\",\"Owner\":\"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb\",\"OfferSequence\":1234,\"LastLedgerSequence\":8820051,\"Fee\":\"1200\",\"Sequence\":23}",
  "instructions": {
    "fee": "0.000012",
    "sequence": 23,
    "maxLedgerVersion": 8820051
...
```json
{
  "txJSON": "{\"Flags\":2147483648,\"TransactionType\":\"EscrowFinish\",\"Account\":\"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb\",\"Owner\":\"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb\",\"OfferSequence\":1234,\"Condition\":\"A0258020E3B0C44298FC1C149AFBF4C8996FB92427AE41E4649B934CA495991B7852B855810100\",\"Fulfillment\":\"A0028000\",\"LastLedgerSequence\":8820051,\"Fee\":\"1584\",\"Sequence\":23}",
  "instructions": {
    "fee": "0.00001584",
    "sequence": 23,
    "maxLedgerVersion": 8820051
...
```json
{
  "txJSON":"{\"Account\":\"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb\",\"TransactionType\":\"PaymentChannelCreate\",\"Amount\":\"100000000\",\"Destination\":\"cJcyvdykuUychWqNehMyN1BqbxF8xcfdhw\",\"SettleDelay\":86400,\"PublicKey\":\"32D2471DB72B27E3310F355BB33E339BF26F8392D5A93D3BC0FC3B566612DA0F0A\",\"Flags\":2147483648,\"LastLedgerSequence\":8820051,\"Fee\":\"1200\",\"Sequence\":23}",
  "instructions": {
    "fee": "0.000012",
    "sequence": 23,
    "maxLedgerVersion": 8820051
...
```json
{
  "txJSON": "{\"Account\":\"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb\",\"TransactionType\":\"PaymentChannelClaim\",\"Channel\":\"C1AE6DDDEEC05CF2978C0BAD6FE302948E9533691DC749DCDD3B9E5992CA6198\",\"Flags\":2147483648,\"LastLedgerSequence\":8820051,\"Fee\":\"1200\",\"Sequence\":23}",
  "instructions": {
    "fee": "0.000012",
    "sequence": 23,
    "maxLedgerVersion": 8820051
...
```json
{
  "txJSON":"{\"Account\":\"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb\",\"TransactionType\":\"PaymentChannelFund\",\"Channel\":\"C1AE6DDDEEC05CF2978C0BAD6FE302948E9533691DC749DCDD3B9E5992CA6198\",\"Amount\":\"100000000\",\"Flags\":2147483648,\"LastLedgerSequence\":8820051,\"Fee\":\"1200\",\"Sequence\":23}",
  "instructions": {
    "fee": "0.000012",
    "sequence": 23,
    "maxLedgerVersion": 8820051
...
```json
{
  "txJSON": "{\"Account\":\"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb\",\"TransactionType\":\"CheckCreate\",\"Destination\":\"cJcyvdykuUychWqNehMyN1BqbxF8xcfdhw\",\"SendMax\":\"100000000\",\"Flags\":2147483648,\"LastLedgerSequence\":8820051,\"Sequence\":23,\"Fee\":\"1200\"}",
  "instructions": {
    "fee": "0.000012",
    "sequence": 23,
    "maxLedgerVersion": 8820051
...
```json
{
  "txJSON": "{\"Account\":\"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb\",\"TransactionType\":\"CheckCancel\",\"CheckID\":\"49647F0D748DC3FE26BDACBC57F251AADEFFF391403EC9BF87C97F67E9977FB0\",\"Flags\":2147483648,\"LastLedgerSequence\":8819954,\"Fee\":\"1200\",\"Sequence\":23}",
  "instructions": {
    "fee": "0.000012",
    "sequence": 23,
    "maxLedgerVersion": 8819954
...
```json
{
  "txJSON": "{\"Account\":\"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb\",\"TransactionType\":\"CheckCash\",\"CheckID\":\"838766BA2B995C00744175F69A1B11E32C3DBC40E64801A4056FCBD657F57334\",\"Amount\":\"100000000\",\"Flags\":2147483648,\"LastLedgerSequence\":8819954,\"Sequence\":23,\"Fee\":\"1200\"}",
  "instructions": {
    "fee": "0.000012",
    "sequence": 23,
    "maxLedgerVersion": 8819954
...
*options.* signAs | [address](#casinocoin-address) | *Optional* The account that the signature should count for in multisigning.
...
```json
{
  "signedTransaction": "",
  "id": "65469B8745DA8AD3B53E6DE410081553E451EEBB03D10DA054C5E7A980CE28FB"
}
```
...
engine_result | string | Code indicating the preliminary result of the transaction, for example tesSUCCESS. [List of transaction responses](https://casinocoin.org/build/transactions/#full-transaction-response-list)
...
tx_json | [tx](https://casinocoin.org/build/transactions/) | The complete transaction in JSON format.

### Example

```javascript
const signedTransaction = '';
...
```json
{
  ...
  "tx_json": {
    "Account": "cMBekxwMUgNov7zVFkoUxACVvu3EE8EZpv",
    "Amount": {
      "currency": "USD",
      "issuer": "cMBekxwMUgNov7zVFkoUxACVvu3EE8EZpv",
      "value": "1"
    },
    "Destination": "csXSWM76NEvYzsYWszjB2fLg8m4fUBaT6C",
    "Fee": "10000",
    "Flags": 2147483648,
    "Sequence": 360,
...
```json
{
  "address": "cGCkuB7PBc5tNy68tPEABEtrdno4hE6Y7f",
...
Derive an CSC Ledger address from a public key.
...
```json
"3045022100F6C4FA988AD3DDCACF56594CB5F9AE2A3C777112D49F3D71B07CE285667BBF70022035E6E1F8704824DF0B101EA9CB9A02B9FC835FC894C1A3ACA7AFFE9735089AEE"
```
...
const channel =
  '3E18C05AD40319B809520F1A136370C4075321B285217323396D6FD9EE1E9037';
const amount = '.00001';
const signature = "3045022100F6C4FA988AD3DDCACF56594CB5F9AE2A3C777112D49F3D71B07CE285667BBF70022035E6E1F8704824DF0B101EA9CB9A02B9FC835FC894C1A3ACA7AFFE9735089AEE";
const publicKey =
...
`cscToDrops(csc: string | BigNumber): string`

Converts an CSC amount to drops. 1 CSC = 100,000,000 drops, so 1 drop = 0.00000001 CSC. This method is useful when converting amounts for use with the casinocoind API, which requires CSC amounts to be specified in drops.

### Parameters

`csc`: A string or BigNumber representing an amount of CSC. If `csc` is a string, it may start with `-`, must contain at least one number, and may contain up to one `.`. This method throws a `ValidationError` for invalid input.
...
`dropsToCsc(drops: string | BigNumber): string`

Converts an amount of drops to CSC. 1 drop = 0.00000001 CSC, so 1 CSC = 100,000,000 drops. This method is useful when converting amounts from the casinocoind API, which describes CSC amounts in drops.
...
A string representing an equivalent amount of CSC.

### Example

```javascript
return api.dropsToCsc('1');
```
...
`iso8601ToCasinocoinTime(iso8601: string): number`

This method parses a string representation of a date, and returns the number of seconds since the "Casinocoin Epoch" of January 1, 2000 (00:00 UTC).

The Casinocoin Epoch is 946684800 seconds after the Unix Epoch.

This method is useful for creating timestamps to use with the casinocoind APIs. The casinocoind APIs represent time as an unsigned integer of the number of seconds since the Casinocoin Epoch.
...
The number of seconds since the Casinocoin Epoch.

### Example

```javascript
api.iso8601ToCasinocoinTime('2017-02-17T15:04:57Z');
```
...
`txFlags.Universal.FullyCanonicalSig`: Require a fully-canonical signature. When preparing transactions, casinocoin-libjs enables this flag for you.

### Payment Flags

`txFlags.Payment.NoCasinocoinDirect`: Do not use the default path; only use specified paths. This is intended to force the transaction to take arbitrage opportunities. Most clients do not need this.

`txFlags.Payment.PartialPayment`: If the specified destination amount cannot be sent without spending more than the source maxAmount, reduce the received amount instead of failing outright. See [Partial Payments](https://casinocoin.org/build/concept-partial-payments.html) for more details.

`txFlags.Payment.LimitQuality`: Only take paths where all the conversions have an input:output ratio that is equal or better than the ratio of `destination.amount`:`source.maxAmount`. See [Limit Quality](https://casinocoin.org/build/reference-transaction-format.html#limit-quality) for details.
...
`txFlags.OfferCreate.Sell`: Treat the offer as a Sell order. With `order.direction = 'sell'`, exchange the entire `order.quantity`, even if it means obtaining more than the `order.totalPrice` amount in exchange. If using `prepareOrder`, casinocoin-libjs sets this flag for you.
...
`txFlags.TrustSet.NoCasinocoin`:  Obsolete.

`txFlags.TrustSet.SetNoCasinocoin`: Blocks [rippling](https://casinocoin.org/build/concept-nocasinocoin.html) between two trustlines of the same currency, if this flag is set on both.

`txFlags.TrustSet.ClearNoCasinocoin`: Clears the No-[Rippling](https://casinocoin.org/build/concept-nocasinocoin.html) flag.

`txFlags.TrustSet.SetFreeze`: Freeze the trustline. A non-CSC currency can be frozen by the exchange or gateway that issued it. CSC cannot be frozen.
...
In the casinocoind API, Account Flags can be enabled and disabled with the SetFlag and ClearFlag parameters. See [AccountSet Flags](https://casinocoin.org/build/reference-ledger-format.html#accountroot-flags).
...
* `txFlags.AccountSet.DisallowCSC`
* `txFlags.AccountSet.AllowCSC`
...
Unlike the rest of the casinocoin-libjs API, schemaValidator is a static object on CasinocoinAPI. It provides utility methods that do not use a server.

## schemaValidate

`CasinocoinAPI.schemaValidator.schemaValidate(schemaName: string, object: any): void`
...
```javascript
CasinocoinAPI.schemaValidator.schemaValidate('sign', {
    signedTransaction: '',
    id: '65469B8745DA8AD3B53E6DE410081553E451EEBB03D10DA054C5E7A980CE28FB'
})
```
