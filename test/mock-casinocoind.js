...
function createResponse(request, response, overrides = {}) {
    ...
    const change = response.result && !_.isEmpty(overrides) ? { id: request.id, result: result } : { id: request.id };
...
module.exports = function createMockCasinocoind(port) {
    ...
    mock.on('request_ledger', function(request, conn) {
        ...
        if (request.ledger_index === 34) {
            ...
            const response = _.assign({}, fixtures.ledger.normal, { result: { ledger: fullLedger } });
    ...
    mock.on('request_casinocoin_path_find', function(request, conn) {
        let response = null;
