...
const fixtures = require('./fixtures/casinocoind');
const addresses = require('./fixtures/addresses');
const hashes = require('./fixtures/hashes');
const transactionsResponse = require('./fixtures/casinocoind/account-tx');
const accountLinesResponse = require('./fixtures/casinocoind/account-lines');
const accountObjectsResponse = require('./fixtures/casinocoind/account-objects');
const fullLedger = require('./fixtures/casinocoind/ledger-full-38129.json');
...
module.exports = function createMockCasinocoind(port) {
  ...
  mock.on('request_book_offers', function (request, conn) {
    if (request.taker_pays.issuer === 'c9QfjU3Z1WLrGuEMnxmaXXQMNT4GBu8xi8') {
      conn.send(createResponse(request, fixtures.book_offers.csc_usd));
    } else if (request.taker_gets.issuer
      === 'c9QfjU3Z1WLrGuEMnxmaXXQMNT4GBu8xi8') {
      conn.send(createResponse(request, fixtures.book_offers.usd_csc));
      ...
      const casinocoindDir = 'test/fixtures/casinocoind';
      if (!requestsCache) {
        requestsCache = fs.readdirSync(casinocoindDir + '/requests');
      }
      for (var i = 0; i < requestsCache.length; i++) {
        const file = requestsCache[i];
        const json = fs.readFileSync(casinocoindDir + '/requests/' + file, 'utf8');
        ...
        if (JSON.stringify(requestWithoutId) === JSON.stringify(r)) {
          const responseFile = casinocoindDir + '/responses/' + file.split('.')[0] + '-res.json';
  ...
  mock.on('request_ripple_path_find', function (request, conn) {
    ...
    if (request.source_account === 'cBYbZHeRgNn1buXHatjvqu7zi6QfPyYrLr') {
      ...
      response = createResponse(request, {
        ...
        "result": {
          "alternatives": [
            {
              "destination_amount": {
                "currency": "EUR",
                "issuer": "chtF5fMTpLykSmHcgCpeBfYBQPma8RzkHB",
                "value": "1"
              },
              "paths_canonical": [],
              "paths_computed": [
          ...
          "destination_account": "cpSCg5AxhGcjEdFbNKYiy4F4Fr7Doif5Z6",
          "destination_amount": {
            "currency": "EUR",
            "issuer": "chtF5fMTpLykSmHcgCpeBfYBQPma8RzkHB",
            "value": "-1"
          ...
          "full_reply": true,
          "id": 2,
          "source_account": "cBYbZHeRgNn1buXHatjvqu7zi6QfPyYrLr",
          "status": "success"
        }
      })
      ...
      response = createResponse(request, fixtures.path_find.CscToCsc, {
        destination_amount: request.destination_amount,
        destination_address: request.destination_address
      });
    } else if (request.source_account === addresses.ACCOUNT) {
      if (request.destination_account === 'csXSWM76NEvYzsYWszjB2fLg8m4fUBaT6C' &&
