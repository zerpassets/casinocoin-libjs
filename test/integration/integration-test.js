...
const {isValidAddress} = require('casinocoin-libjs-address-codec');
...
function setup(server = 'wss://s1.casinocoin.org') {
  this.api = new CasinocoinAPI({server});
...
const masterAccount = 'cLKccc6TfhjG1G4ySAUuWNAkUSzwUer8Lj';
const masterSecret = 'ssvkaUEUiqVoVBLxSPYEs2kFnuFZ4';
...
function setupAccounts(testcase) {
  ...
  const promise = payTo(api, 'cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne')
    .then(() => payTo(api, wallet.getAddress()))
    .then(() => payTo(api, testcase.newWallet.address))
    .then(() => payTo(api, 'cM1beXdZHZLiiMXBx7bqYUdsjVmHjDyNaZ'))
    .then(() => payTo(api, 'chvDYvKFV7iNXezNLTrMfM5Dre1Hu4vpZQ'))
    .then(() => {
      return api.prepareSettings(masterAccount, {defaultCasinocoin: true})
...
describe('integration tests', function() {
  ...
  it('payment', function() {
    const amount = {currency: 'CSC', value: '0.00000001'};
    const paymentSpecification = {
      ...
      destination: {
        address: 'cM1beXdZHZLiiMXBx7bqYUdsjVmHjDyNaZ',
        amount: amount
      }
    };
    return this.api.getLedgerVersion().then(ledgerVersion => {
  ...
  it('getOrderbook', function() {
    ...
    return this.api.getOrderbook(address, orderbook).then(book => {
      ...
      assert.strictEqual(bid.specification.quantity.currency, 'CSC');
      ...
      assert.strictEqual(ask.specification.quantity.currency, 'CSC');
...
describe('integration tests - standalone casinocoind', function() {
  const instructions = {maxLedgerVersionOffset: 10};
  this.timeout(TIMEOUT);

  beforeEach(_.partial(setup, serverUrl));
  afterEach(teardown);
  const address = 'c3jV8vYKVtZTccXY66835nkNnxqcfcvxGo';
  const secret = 'ss6F8381Br6wwpy9p582H8sBt19J3';
  const signer1address = 'cBFUmfFUeo53yGGDuGMpdayKBT1vTDhnpe';
  const signer1secret = 'shK6YXzwYfnFVn3YZSaMh5zuAddKx';
  const signer2address = 'chjPTZ1xLyYmMDnaJzUfGNcqXwMEJyGWsZ';
