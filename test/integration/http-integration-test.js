...
const CasinocoinAPI = require('../../src').CasinocoinAPI;
...
describe('http server integration tests', function() {
    ...
    before(() => {
        this.api = new CasinocoinAPI({ server: serverUri });
    ...
    createTest(
        'sign', [{ txJSON: apiRequests.sign.normal.txJSON },
    ...
    createTest(
        'generateAddress', [{ options: { entropy: random() } }],
        result => assert.deepEqual(result.result, apiResponses.generateAddress)
    );

    createTest(
        'computeLedgerHash', [{
            ledger: _.assign({}, apiResponses.getLedger.full, { parentCloseTime: apiResponses.getLedger.full.closeTime })
        }],
        result => {
