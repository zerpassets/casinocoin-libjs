...
const CasinocoinAPI = require('casinocoin-api').CasinocoinAPI;
...
describe('CasinocoinAPI', function () {
  ...
  describe('cscToDrops', function () {
    it('works with a typical amount', function () {
      const drops = this.api.cscToDrops('2')
      assert.strictEqual(drops, '200000000', '2 CSC equals 200 million drops')
    })

    it('works with fractions', function () {
      let drops = this.api.cscToDrops('3.456789')
      assert.strictEqual(drops, '345678900', '3.45678900 CSC equals 345,678,900 drops')

      drops = this.api.cscToDrops('3.400000')
      assert.strictEqual(drops, '340000000', '3.40000000 CSC equals 340,000,000 drops')

      drops = this.api.cscToDrops('0.00000001')
      assert.strictEqual(drops, '1', '0.00000001 CSC equals 1 drop')

      drops = this.api.cscToDrops('0.000000010')
      assert.strictEqual(drops, '1', '0.000000010 CSC equals 1 drop')
    })

    it('works with zero', function () {
      let drops = this.api.cscToDrops('0')
      assert.strictEqual(drops, '0', '0 CSC equals 0 drops')

      // negative zero is equivalent to zero
      drops = this.api.cscToDrops('-0')
      assert.strictEqual(drops, '0', '-0 CSC equals 0 drops')

      drops = this.api.cscToDrops('0.000000')
      assert.strictEqual(drops, '0', '0.000000 CSC equals 0 drops')

      drops = this.api.cscToDrops('0.0000000')
      assert.strictEqual(drops, '0', '0.0000000 CSC equals 0 drops')
    })
    ...
    it('works with a value ending with a decimal point', function () {
      let drops = this.api.cscToDrops('2.')
      assert.strictEqual(drops, '200000000', '2. CSC equals 200000000 drops')

      drops = this.api.cscToDrops('-2.')
      assert.strictEqual(drops, '-200000000', '-2. CSC equals -200000000 drops')
    })

    it('works with BigNumber objects', function () {
      let drops = this.api.cscToDrops(new BigNumber(2))
      assert.strictEqual(drops, '200000000', '(BigNumber) 2 CSC equals 200 million drops')

      drops = this.api.cscToDrops(new BigNumber(-2))
      assert.strictEqual(drops, '-200000000', '(BigNumber) -2 CSC equals -200 million drops')
    })
    ...
    it('works with a number', function() {
      ...
      let drops = this.api.cscToDrops(2)
      assert.strictEqual(drops, '200000000', '(number) 2 CSC equals 200 million drops')
    ...
    it('throws with an amount with too many decimal places', function () {
      assert.throws(() => {
        this.api.cscToDrops('1.123456789')
      }, /has too many decimal places/)

      assert.throws(() => {
        this.api.cscToDrops('0.000000001')
      }, /has too many decimal places/)
    })

    it('throws with an invalid value', function () {
      assert.throws(() => {
        this.api.cscToDrops('FOO')
      }, /invalid value/)

      assert.throws(() => {
        this.api.cscToDrops('1e-7')
      }, /invalid value/)

      assert.throws(() => {
        this.api.cscToDrops('2,0')
      }, /invalid value/)

      assert.throws(() => {
        this.api.cscToDrops('.')
      }, /cscToDrops\: invalid value '\.', should be a BigNumber or string-encoded number\./)
    })

    it('throws with an amount more than one decimal point', function () {
      assert.throws(() => {
        this.api.cscToDrops('1.0.0')
      }, /cscToDrops:\ invalid\ value\ '1\.0\.0'\,\ should\ be\ a\ number\ matching\ \(\^\-\?\[0\-9\]\*\.\?\[0\-9\]\*\$\)\./)

      assert.throws(() => {
        this.api.cscToDrops('...')
      }, /cscToDrops:\ invalid\ value\ '\.\.\.'\,\ should\ be\ a\ number\ matching\ \(\^\-\?\[0\-9\]\*\.\?\[0\-9\]\*\$\)\./)
    })
  })

  describe('dropsToCsc', function () {
    it('works with a typical amount', function () {
      const csc = this.api.dropsToCsc('200000000')
      assert.strictEqual(csc, '2', '200 million drops equals 2 CSC')
    })

    it('works with fractions', function () {
      let csc = this.api.dropsToCsc('345678900')
      assert.strictEqual(csc, '3.456789', '345,678,900 drops equals 3.456789 CSC')

      csc = this.api.dropsToCsc('340000000')
      assert.strictEqual(csc, '3.4', '300,400,000 drops equals 3.4 CSC')

      csc = this.api.dropsToCsc('1')
      assert.strictEqual(csc, '0.00000001', '1 drop equals 0.00000001 CSC')

      csc = this.api.dropsToCsc('1.0')
      assert.strictEqual(csc, '0.00000001', '1.0 drops equals 0.00000001 CSC')

      csc = this.api.dropsToCsc('1.00')
      assert.strictEqual(csc, '0.00000001', '1.00 drops equals 0.00000001 CSC')
    })

    it('works with zero', function () {
      let csc = this.api.dropsToCsc('0')
      assert.strictEqual(csc, '0', '0 drops equals 0 CSC')

      // negative zero is equivalent to zero
      csc = this.api.dropsToCsc('-0')
      assert.strictEqual(csc, '0', '-0 drops equals 0 CSC')

      csc = this.api.dropsToCsc('0.00')
      assert.strictEqual(csc, '0', '0.00 drops equals 0 CSC')

      csc = this.api.dropsToCsc('000000000')
      assert.strictEqual(csc, '0', '000000000 drops equals 0 CSC')
    })
    ...
    it('works with a value ending with a decimal point', function () {
      let csc = this.api.dropsToCsc('200000000.')
      assert.strictEqual(csc, '2', '200000000. drops equals 2 CSC')

      csc = this.api.dropsToCsc('-200000000.')
      assert.strictEqual(csc, '-2', '-200000000. drops equals -2 CSC')
    })

    it('works with BigNumber objects', function () {
      let csc = this.api.dropsToCsc(new BigNumber(200000000))
      assert.strictEqual(csc, '2', '(BigNumber) 200 million drops equals 2 CSC')

      csc = this.api.dropsToCsc(new BigNumber(-200000000))
      assert.strictEqual(csc, '-2', '(BigNumber) -200 million drops equals -2 CSC')

      csc = this.api.dropsToCsc(new BigNumber(234567809))
      assert.strictEqual(csc, '2.34567809', '(BigNumber) 234,567,809 drops equals 2.34567809 CSC')
    ...
    it('works with a number', function() {
      ...
      let csc = this.api.dropsToCsc(200000000)
      assert.strictEqual(csc, '2', '(number) 200 million drops equals 2 CSC')
    ...
    it('throws with an amount with too many decimal places', function () {
      assert.throws(() => {
        this.api.dropsToCsc('1.2')
      }, /has too many decimal places/)

      assert.throws(() => {
        this.api.dropsToCsc('0.10')
      }, /has too many decimal places/)
    })

    it('throws with an invalid value', function () {
      assert.throws(() => {
        this.api.dropsToCsc('FOO')
      }, /invalid value/)

      assert.throws(() => {
        this.api.dropsToCsc('1e-7')
      }, /invalid value/)

      assert.throws(() => {
        this.api.dropsToCsc('2,0')
      }, /invalid value/)

      assert.throws(() => {
        this.api.dropsToCsc('.')
      }, /dropsToCsc\: invalid value '\.', should be a BigNumber or string-encoded number\./)
    })

    it('throws with an amount more than one decimal point', function () {
      assert.throws(() => {
        this.api.dropsToCsc('1.0.0')
      }, /dropsToCsc:\ invalid\ value\ '1\.0\.0'\,\ should\ be\ a\ number\ matching\ \(\^\-\?\[0\-9\]\*\.\?\[0\-9\]\*\$\)\./)

      assert.throws(() => {
        this.api.dropsToCsc('...')
      }, /dropsToCsc:\ invalid\ value\ '\.\.\.'\,\ should\ be\ a\ number\ matching\ \(\^\-\?\[0\-9\]\*\.\?\[0\-9\]\*\$\)\./)
    })
  })

  describe('isValidAddress', function () {
    it('returns true for valid address', function () {
      assert(this.api.isValidAddress('cJMPuiM2sHvBqzRsoBPVcAJhobfkguA1Wb'));
    })
  ...
  describe('isValidSecret', function () {
    it('returns true for valid secret', function () {
      assert(this.api.isValidSecret('snBiLvuQcNFo6t1vhEwpJ8tcNSfPp'));
    })
  ...
  describe('deriveKeypair', function () {
    it('returns keypair for secret', function () {
      var keypair = this.api.deriveKeypair('snBiLvuQcNFo6t1vhEwpJ8tcNSfPp');
      assert.equal(keypair.privateKey, '00E0A5116257AA80FDABC5320E38DC0208DE5AF57AD85F80C8F742E52C70F944ED');
      assert.equal(keypair.publicKey, '03064E7AA15A1D967954799A2C1702072BF9DE65EFF066AE4194C704C8A5067740');
    })
  ...
  describe('deriveAddress', function () {
    it('returns address for public key', function () {
      var address = this.api.deriveAddress('03064E7AA15A1D967954799A2C1702072BF9DE65EFF066AE4194C704C8A5067740');
      assert.equal(address, 'cJMPuiM2sHvBqzRsoBPVcAJhobfkguA1Wb');
    })
  })
  ...
  describe('preparePayment', function () {
    ...
    it('preparePayment - CSC to CSC', function () {
      const payment = {
        "source": {
      ...
      return this.api.preparePayment(address, payment, instructions).then(response => {
        const expected = {
          txJSON: '{"TransactionType":"Payment","Account":"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb","Destination":"chnD92a3R6Eyr2x7ujNmS7GnwFDf3FrFbr","Amount":"100000000","Flags":2147483648,"LastLedgerSequence":8820051,"Sequence":23,"Fee":"1200"}',
          instructions: {
            fee: '0.000012',
            sequence: 23,
    ...
    it('preparePayment - CSC drops to CSC drops', function () {
      const payment = {
        "source": {
          "address": "cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb",
          "maxAmount": {
    ...
    it('preparePayment - CSC drops to CSC', function () {
      const payment = {
        "source": {
          "address": "cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb",
          "maxAmount": {
    ...
    it('preparePayment - CSC to CSC drops', function () {
      const payment = {
        "source": {
          "address": "cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb",
          "maxAmount": {
    ...
    it('preparePayment - throws when fee exceeds 2 CSC', function () {
      const localInstructions = _.defaults({
        fee: '2.1'
      }, instructions);
    ...
    it('preparePayment - allows fee exceeding 2 CSC when maxFeeCSC is higher', function () {
      this.api._maxFeeCSC = '2.2'
      const localInstructions = _.defaults({
        fee: '2.1'
      }, instructions);

      const expectedResponse = {
        "txJSON": "{\"Flags\":2147483648,\"TransactionType\":\"Payment\",\"Account\":\"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb\",\"Destination\":\"chnD92a3R6Eyr2x7ujNmS7GnwFDf3FrFbr\",\"Amount\":{\"value\":\"0.01\",\"currency\":\"USD\",\"issuer\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"},\"SendMax\":{\"value\":\"0.01\",\"currency\":\"USD\",\"issuer\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"},\"LastLedgerSequence\":8820051,\"Fee\":\"210000000\",\"Sequence\":23}",
        "instructions": {
          "fee": "2.1",
          "sequence": 23,
          "maxLedgerVersion": 8820051
  ...
  it('prepareSettings - regularKey', function () {
    const regularKey = { regularKey: 'cGo8mMx4AGvKxauVy1FEBrrLH3Yf7raHuP' };
  ...
  it('prepareTransaction - DepositPreauth - Authorize', function () {
    ...
    const txJSON = {
      ...
      Account: address,
      Authorize: 'chnD92a3R6Eyr2x7ujNmS7GnwFDf3FrFbr'
    }

    return this.api.prepareTransaction(txJSON, localInstructions).then(response => {
      const expected = {
        txJSON: '{"TransactionType":"DepositPreauth","Account":"' + address + '","Authorize":"chnD92a3R6Eyr2x7ujNmS7GnwFDf3FrFbr","Flags":2147483648,"LastLedgerSequence":8820051,"Fee":"1200","Sequence":23}',
        instructions: {
          fee: '0.000012',
          sequence: 23,
          maxLedgerVersion: 8820051
  ...
  it('prepareTransaction - DepositPreauth - Unauthorize', function () {
    ...
    const txJSON = {
      TransactionType: 'DepositPreauth',
      Account: address,
      Unauthorize: 'chnD92a3R6Eyr2x7ujNmS7GnwFDf3FrFbr'
    }

    return this.api.prepareTransaction(txJSON, localInstructions).then(response => {
      const expected = {
        txJSON: '{"TransactionType":"DepositPreauth","Account":"' + address + '","Unauthorize":"chnD92a3R6Eyr2x7ujNmS7GnwFDf3FrFbr","Flags":2147483648,"LastLedgerSequence":8820051,"Fee":"1200","Sequence":23}',
        instructions: {
          fee: '0.000012',
          sequence: 23,
          maxLedgerVersion: 8820051
  ...
  describe('prepareTransaction - Payment', function () {
    ...
    it('min amount csc', function () {
      ...
      const txJSON = {
        ...
        // Max amount to send. Use 100 billion CSC to
    ...
    // prepareTransaction - Payment
    it('min amount csc2csc', function () {
      const txJSON = {
        TransactionType: 'Payment',
        Account: address,
        Destination: 'chnD92a3R6Eyr2x7ujNmS7GnwFDf3FrFbr',
    ...
    it('with all options specified', function () {
      return this.api.getLedgerVersion().then(ver => {
        ...
        const txJSON = {
          ...
          Flags: 0 | this.api.txFlags.Payment.NoCasinocoinDirect | this.api.txFlags.Payment.LimitQuality
        }
        return this.api.prepareTransaction(txJSON, localInstructions).then(
            _.partial(checkResult,
              responses.preparePayment.allOptions, 'prepare'));
    ...
    // prepareTransaction - Payment
    it('fee is capped at default maxFee of 2 CSC', function () {
      this.api._feeCushion = 1000000;
    ...
    // prepareTransaction - Payment
    it('fee is capped to custom maxFeeCSC when maxFee exceeds maxFeeCSC', function () {
      this.api._feeCushion = 1000000
      this.api._maxFeeCSC = '3'
      const localInstructions = _.defaults({
        maxFee: '4' // We are testing that this does not matter; fee is still capped to maxFeeCSC
      }, instructions);
      ...
      const expectedResponse = {
        "txJSON": "{\"Flags\":2147483648,\"TransactionType\":\"Payment\",\"Account\":\"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb\",\"Destination\":\"chnD92a3R6Eyr2x7ujNmS7GnwFDf3FrFbr\",\"Amount\":{\"value\":\"0.01\",\"currency\":\"USD\",\"issuer\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"},\"SendMax\":{\"value\":\"0.01\",\"currency\":\"USD\",\"issuer\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"},\"LastLedgerSequence\":8820051,\"Fee\":\"300000000\",\"Sequence\":23}",
        "instructions": {
          "fee": "3",
          "sequence": 23,
          "maxLedgerVersion": 8820051
    ...
    it('fee is capped to maxFee', function () {
      this.api._feeCushion = 1000000
      this.api._maxFeeCSC = '5'
      const localInstructions = _.defaults({
        maxFee: '4' // maxFeeCSC does not matter if maxFee is lower than maxFeeCSC
      }, instructions);
      ...
      const expectedResponse = {
        "txJSON": "{\"Flags\":2147483648,\"TransactionType\":\"Payment\",\"Account\":\"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb\",\"Destination\":\"chnD92a3R6Eyr2x7ujNmS7GnwFDf3FrFbr\",\"Amount\":{\"value\":\"0.01\",\"currency\":\"USD\",\"issuer\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"},\"SendMax\":{\"value\":\"0.01\",\"currency\":\"USD\",\"issuer\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"},\"LastLedgerSequence\":8820051,\"Fee\":\"400000000\",\"Sequence\":23}",
        "instructions": {
          "fee": "4",
          "sequence": 23,
          "maxLedgerVersion": 8820051
    ...
    it('fee - calculated fee does not use more than 6 decimal places', function () {
      ...
      const expectedResponse = {
        "txJSON": "{\"Flags\":2147483648,\"TransactionType\":\"Payment\",\"Account\":\"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb\",\"Destination\":\"chnD92a3R6Eyr2x7ujNmS7GnwFDf3FrFbr\",\"Amount\":{\"value\":\"0.01\",\"currency\":\"USD\",\"issuer\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"},\"SendMax\":{\"value\":\"0.01\",\"currency\":\"USD\",\"issuer\":\"cLdqZR9Y2f3MmgkTbbhhc24YSAySnXbUne\"},\"LastLedgerSequence\":8820051,\"Fee\":\"6489600\",\"Sequence\":23}",
        "instructions": {
          "fee": "0.064896",
          "sequence": 23,
          "maxLedgerVersion": 8820051
  ...
  it('prepareTransaction - PaymentChannelCreate', function () {
    ...
    return this.api.prepareTransaction({
      ...
      Amount: '100000000', // 1 CSC in drops. Use a string-encoded integer.
      Destination: 'cJcyvdykuUychWqNehMyN1BqbxF8xcfdhw',
      SettleDelay: 86400,
      PublicKey: '32D2471DB72B27E3310F355BB33E339BF26F8392D5A93D3BC0FC3B566612DA0F0A'
      // If cancelAfter is used, you must use CasinocoinTime.
      // You can use `iso8601ToCasinocoinTime()` to convert to CasinocoinTime.
  ...
  it('prepareTransaction - PaymentChannelCreate full', function () {
    const txJSON = {
      Account: address,
      TransactionType: 'PaymentChannelCreate',
      Amount: this.api.cscToDrops('1'), // or '1000000'
      Destination: 'cJcyvdykuUychWqNehMyN1BqbxF8xcfdhw',
      SettleDelay: 86400,
      ...
      CancelAfter: this.api.iso8601ToCasinocoinTime('2017-02-17T15:04:57Z'),
      SourceTag: 11747,
  ...
  it('prepareTransaction - PaymentChannelFund full', function () {
    const txJSON = {
      ...
      Channel: 'C1AE6DDDEEC05CF2978C0BAD6FE302948E9533691DC749DCDD3B9E5992CA6198',
      Amount: this.api.cscToDrops('1'), // or '1000000'
      Expiration: this.api.iso8601ToCasinocoinTime('2017-02-17T15:04:57Z')
    }
  ...
  it('prepareTransaction - PaymentChannelClaim with renew', function () {
    ...
    const txJSON = {
      ...
      TransactionType: 'PaymentChannelClaim',
      Channel: 'C1AE6DDDEEC05CF2978C0BAD6FE302948E9533691DC749DCDD3B9E5992CA6198',
      Balance: this.api.cscToDrops('1'), // or '1000000'
      Amount: this.api.cscToDrops('1'), // or '1000000'
  ...
  it('sign - signAs', function () {
    ...
    const signature = this.api.sign(JSON.stringify(txJSON), secret,
      { signAs: 'cLKccc6TfhjG1G4ySAUuWNAkUSzwUer8Lj' });
  ...
  it('sign - withKeypair', function () {
    const keypair = {
      privateKey:
        '00DC91BD844CE0CDF936BD0BFEB3BE9BF78E56876188E3B8100807A956CA4D9077',
      publicKey:
        '0223D5F14BABD2A11A7DA7E5566D90122A6B33EAD2A3553EC9504AF990C5F1F335'
    };
  ...
  it('sign - throws when Fee exceeds maxFeeCSC (in drops)', function () {
    const secret = 'ssvkaUEUiqVoVBLxSPYEs2kFnuFZ4';
    const request = {
      "txJSON": "{\"Flags\":2147483648,\"TransactionType\":\"AccountSet\",\"Account\":\"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb\",\"Domain\":\"636173696E6F636F696E2E6F7267\",\"LastLedgerSequence\":8820051,\"Fee\":\"201000000\",\"Sequence\":23,\"SigningPubKey\":\"02F89EAEC7667B30F33D0687BBA86C3FE2A08CCA40A9186C5BDE2DAA6FA97A37D8\"}",
      "instructions": {
        "fee": "200.01",
        "sequence": 23,
        "maxLedgerVersion": 8820051
    ...
    }, /Fee" should not exceed "200000000"\. To use a higher fee, set `maxFeeCSC` in the CasinocoinAPI constructor\./)
  });

  it('sign - throws when Fee exceeds maxFeeCSC (in drops) - custom maxFeeCSC', function () {
    this.api._maxFeeCSC = '1.9'
    const secret = 'ssvkaUEUiqVoVBLxSPYEs2kFnuFZ4';
    const request = {
      "txJSON": "{\"Flags\":2147483648,\"TransactionType\":\"AccountSet\",\"Account\":\"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb\",\"Domain\":\"636173696E6F636F696E2E6F7267\",\"LastLedgerSequence\":8820051,\"Fee\":\"201000000\",\"Sequence\":23,\"SigningPubKey\":\"02F89EAEC7667B30F33D0687BBA86C3FE2A08CCA40A9186C5BDE2DAA6FA97A37D8\"}",
      "instructions": {
    ...
    assert.throws(() => {
      this.api.sign(request.txJSON, secret)
    }, /Fee" should not exceed "190000000"\. To use a higher fee, set `maxFeeCSC` in the CasinocoinAPI constructor\./)
  });

  it('sign - permits fee exceeding 2000000 drops when maxFeeCSC is higher than 2 CSC', function () {
    this.api._maxFeeCSC = '2.1'
    const secret = 'ssvkaUEUiqVoVBLxSPYEs2kFnuFZ4';
    const request = {
      "txJSON": "{\"Flags\":2147483648,\"TransactionType\":\"AccountSet\",\"Account\":\"cLPp8TAuVgFP45vDyrLrG73xmVV6CDJjRb\",\"Domain\":\"636173696E6F636F696E2E6F7267\",\"LastLedgerSequence\":8820051,\"Fee\":\"2010000\",\"Sequence\":23,\"SigningPubKey\":\"02F89EAEC7667B30F33D0687BBA86C3FE2A08CCA40A9186C5BDE2DAA6FA97A37D8\"}",
      "instructions": {
        "fee": "2.01",
        "sequence": 23,
        "maxLedgerVersion": 8820051
    ...
    const expectedResponse =  {
      signedTransaction: "",
      id: "E1D878C4E74BDCF06B055585329FC7D54855F341FA89582282F2FCF22F1E42F8"
    }
    ...
    it('getBalances - limit & currency & issuer', function () {
      ...
      const expectedResponse = _.filter(responses.getBalances,
        item => item.currency === 'USD' &&
          item.counterparty === 'cpa54XMDD4U9SNpx3xDQqDecUX9r6wcbKd').slice(0, 3);
      return this.api.getBalances(address, options).then(
        _.partial(checkResult, expectedResponse, 'getBalances'));
    });
  });
  ...
  describe('getTransaction', () => {
    ...
    it('getTransaction - settings - include raw transaction', function () {
      ...
      expected.rawTransaction = "{\"Account\":\"cLdrZLwgVkaS913Zb1Dra8pKyRtV5V5VMY\",\"Fee\":\"10\",\"Flags\":2147483648,\"Sequence\":1,\"SetFlag\":2,\"SigningPubKey\":\"03EA3ADCA632F125EC2CC4F7F6A82DE0DCE2B65290CAC1F22242C5163F0DA9652D\",\"TransactionType\":\"AccountSet\",\"TxnSignature\":\"3045022100DE8B666B1A31EA65011B0F32130AB91A5747E32FA49B3054CEE8E8362DBAB98A022040CF0CF254677A8E5CD04C59CA2ED7F6F15F7E184641BAE169C561650967B226\",\"date\":460832270,\"hash\":\"4FB3ADF22F3C605E23FAEFAA185F3BD763C4692CAC490D9819D117CD33BFAA1B\",\"inLedger\":8206418,\"ledger_index\":8206418,\"meta\":{\"AffectedNodes\":[{\"ModifiedNode\":{\"FinalFields\":{\"Account\":\"cLdrZLwgVkaS913Zb1Dra8pKyRtV5V5VMY\",\"Balance\":\"29999990\",\"Flags\":786432,\"OwnerCount\":0,\"Sequence\":2},\"LedgerEntryType\":\"AccountRoot\",\"LedgerIndex\":\"3F5072C4875F32ED770DAF3610A716600ED7C7BB0348FADC7A98E011BB2CD36F\",\"PreviousFields\":{\"Balance\":\"30000000\",\"Flags\":4194304,\"Sequence\":1},\"PreviousTxnID\":\"3FB0350A3742BBCC0D8AA3C5247D1AEC01177D0A24D9C34762BAA2FEA8AD88B3\",\"PreviousTxnLgrSeq\":8206397}}],\"TransactionIndex\":5,\"TransactionResult\":\"tesSUCCESS\"},\"validated\":true}"
  ...
  describe('formatBidsAndAsks', function () {
    ...
    it('with CSC', function () {
      const orderbookInfo = {
        "base": {
          "currency": "USD",
          "counterparty": "c9QfjU3Z1WLrGuEMnxmaXXQMNT4GBu8xi8"
        },
        "counter": {
          "currency": "CSC"
        }
      ...
      return Promise.all(
        ...
        const reverseOffers = (reverseOfferResults ? reverseOfferResults : []).reduce((acc, res) => acc.concat(res.offers), [])
        const orderbook = CasinocoinAPI.formatBidsAndAsks(orderbookInfo, [...directOffers, ...reverseOffers]);
        assert.deepEqual(orderbook, responses.getOrderbook.withCSC);
      });
    });
    ...
    it('sample CSC/JPY book has orders sorted correctly', function () {
      const orderbookInfo = {
        ...
        "counter": {
          "currency": 'JPY',
          "counterparty": "cUyJuAXaPwe2TgBEzr1qkQMR6eCw6CT5Vn"
        }
      };

      const myAddress = 'cGRAhAd1H8CUk9KsQzbL6MPeh6jGPTY3zd';
      ...
      return Promise.all(
        [
          this.api.request('book_offers', {
            ...
            taker_pays: CasinocoinAPI.renameCounterpartyToIssuer(orderbookInfo.counter),
            ledger_index: 'validated',
            limit: 400, // must match `test/fixtures/casinocoind/requests/1-taker_gets-CSC-taker_pays-JPY.json`
            taker: myAddress
          }),
          this.api.request('book_offers', {
            taker_gets: CasinocoinAPI.renameCounterpartyToIssuer(orderbookInfo.counter),
            taker_pays: CasinocoinAPI.renameCounterpartyToIssuer(orderbookInfo.base),
            ledger_index: 'validated',
            limit: 400, // must match `test/fixtures/casinocoind/requests/2-taker_gets-JPY-taker_pays-CSC.json`
            taker: myAddress
          })
        ]
      ).then((directOfferResults, reverseOfferResults) => {
    ...
    it('sample USD/CSC book has orders sorted correctly', function () {
      const orderbookInfo = { counter: { currency: 'CSC' },
      base: { currency: 'USD',
       counterparty: 'cpa54XMDD4U9SNpx3xDQqDecUX9r6wcbKd' } };
  ...
  it('getFee - high load_factor with custom maxFeeCSC', function () {
    // Ensure that overriding with high maxFeeCSC of '51540' causes no errors.
    // (fee will actually be 51539.607552)
    this.api._maxFeeCSC = '51540'
    this.api.connection._send(JSON.stringify({
      command: 'config',
      data: { highLoadFactor: true }
    }));
  ...
  it('fee - default maxFee of 2 CSC', function () {
    this.api._feeCushion = 1000000;
  ...
  it('fee - capped to maxFeeCSC when maxFee exceeds maxFeeCSC', function () {
    this.api._feeCushion = 1000000
    this.api._maxFeeCSC = '3'
    const localInstructions = _.defaults({
      maxFee: '4'
  ...
  it('getPaths - result path has source_amount in drops', function () {
    return this.api.getPaths({
      source: {
        address: 'cBYbZHeRgNn1buXHatjvqu7zi6QfPyYrLr',
        amount: {
          value: this.api.dropsToCsc(1000000),
          currency: 'CSC'
        }
      },
      destination: {
        address: 'cpSCg5AxhGcjEdFbNKYiy4F4Fr7Doif5Z6',
        amount: {
          counterparty: 'chtF5fMTpLykSmHcgCpeBfYBQPma8RzkHB',
          currency: 'EUR'
        }
      }
    }).then(
      _.partial(checkResult, [
        {
          ...
          "paths": "[[{\"currency\":\"USD\",\"issuer\":\"chtF5fMTpLykSmHcgCpeBfYBQPma8RzkHB\"},{\"currency\":\"EUR\",\"issuer\":\"chtF5fMTpLykSmHcgCpeBfYBQPma8RzkHB\"}]]"
        }
      ], 'getPaths'));
  });
    ...
    return this.api.getLedger(request).then(
      ...
      .then(response => {
        ...
        assert.strictEqual(hash,
          '16D84B1318C30507C0E2CC57362ADB43AB63095DCEC9AD86D3D08BBA9751B1E0');
      });
  });
  ...
  it('computeLedgerHash - given corrupt data - should fail', function () {
    ...
    return this.api.getLedger(request).then(ledger => {
      assert.strictEqual(ledger.transactions[0].rawTransaction, "{\"Account\":\"chbDYUkaudr5ZcAsmcn9FCJjGHtDWUR6Wd\",\"Amount\":\"10000000000\",\"Destination\":\"cBW1t4BjtdmnWoXQ5shGVpi1rZgASFPFaV\",\"Fee\":\"10\",\"Flags\":0,\"Sequence\":62,\"SigningPubKey\":\"034AADB09CFF4A4804073701EC53C3510CDC95917C2BB0150FB742D0C66E6CEE9E\",\"TransactionType\":\"Payment\",\"TxnSignature\":\"3045022022EB32AECEF7C644C891C19F87966DF9C62B1F34BABA6BE774325E4BB8E2DD62022100A51437898C28C2B297112DF8131F2BB39EA5FE613487DDD611525F1796264639\",\"hash\":\"3B1A4E1C9BB6A7208EB146BCDB86ECEA6068ED01466D933528CA2B4C64F753EF\",\"meta\":{\"AffectedNodes\":[{\"CreatedNode\":{\"LedgerEntryType\":\"AccountRoot\",\"LedgerIndex\":\"4C6ACBD635B0F07101F7FA25871B0925F8836155462152172755845CE691C49E\",\"NewFields\":{\"Account\":\"cBW1t4BjtdmnWoXQ5shGVpi1rZgASFPFaV\",\"Balance\":\"10000000000\",\"Sequence\":1}}},{\"ModifiedNode\":{\"FinalFields\":{\"Account\":\"chbDYUkaudr5ZcAsmcn9FCJjGHtDWUR6Wd\",\"Balance\":\"981481999380\",\"Flags\":0,\"OwnerCount\":0,\"Sequence\":63},\"LedgerEntryType\":\"AccountRoot\",\"LedgerIndex\":\"B33FDD5CF3445E1A7F2BE9B06336BEBD73A5E3EE885D3EF93F7E3E2992E46F1A\",\"PreviousFields\":{\"Balance\":\"991481999390\",\"Sequence\":62},\"PreviousTxnID\":\"2485FDC606352F1B0785DA5DE96FB9DBAF43EB60ECBB01B7F6FA970F512CDA5F\",\"PreviousTxnLgrSeq\":31317}}],\"TransactionIndex\":0,\"TransactionResult\":\"tesSUCCESS\"},\"ledger_index\":38129}");

      // Change Amount to 12000000000
      ledger.transactions[0].rawTransaction = "{\"Account\":\"chbDYUkaudr5ZcAsmcn9FCJjGHtDWUR6Wd\",\"Amount\":\"12000000000\",\"Destination\":\"cBW1t4BjtdmnWoXQ5shGVpi1rZgASFPFaV\",\"Fee\":\"10\",\"Flags\":0,\"Sequence\":62,\"SigningPubKey\":\"034AADB09CFF4A4804073701EC53C3510CDC95917C2BB0150FB742D0C66E6CEE9E\",\"TransactionType\":\"Payment\",\"TxnSignature\":\"3045022022EB32AECEF7C644C891C19F87966DF9C62B1F34BABA6BE774325E4BB8E2DD62022100A51437898C28C2B297112DF8131F2BB39EA5FE613487DDD611525F1796264639\",\"hash\":\"3B1A4E1C9BB6A7208EB146BCDB86ECEA6068ED01466D933528CA2B4C64F753EF\",\"meta\":{\"AffectedNodes\":[{\"CreatedNode\":{\"LedgerEntryType\":\"AccountRoot\",\"LedgerIndex\":\"4C6ACBD635B0F07101F7FA25871B0925F8836155462152172755845CE691C49E\",\"NewFields\":{\"Account\":\"cBW1t4BjtdmnWoXQ5shGVpi1rZgASFPFaV\",\"Balance\":\"10000000000\",\"Sequence\":1}}},{\"ModifiedNode\":{\"FinalFields\":{\"Account\":\"chbDYUkaudr5ZcAsmcn9FCJjGHtDWUR6Wd\",\"Balance\":\"981481999380\",\"Flags\":0,\"OwnerCount\":0,\"Sequence\":63},\"LedgerEntryType\":\"AccountRoot\",\"LedgerIndex\":\"B33FDD5CF3445E1A7F2BE9B06336BEBD73A5E3EE885D3EF93F7E3E2992E46F1A\",\"PreviousFields\":{\"Balance\":\"991481999390\",\"Sequence\":62},\"PreviousTxnID\":\"2485FDC606352F1B0785DA5DE96FB9DBAF43EB60ECBB01B7F6FA970F512CDA5F\",\"PreviousTxnLgrSeq\":31317}}],\"TransactionIndex\":0,\"TransactionResult\":\"tesSUCCESS\"},\"ledger_index\":38129}";
      ...
      try {
        ...
        assert.deepStrictEqual(error.data, {
          transactionHashInHeader: '437782CBE68AE7874C7C9AB2586B5B5620660829A8D3EDC28871920DDE0B64E8',
          computedHashOfTransactions: '2680B0F6CD40D3412A2BEF72A4D38FAF8BF6D18A52143D7B4A9FF978FA07AC18'
        });
        return;
      }
...
describe('CasinocoinAPI - offline', function () {
  ...
  it('computeLedgerHash - with transactions', function () {
    ...
    const ledgerHash = api.computeLedgerHash(header);
    assert.strictEqual(ledgerHash,
      '04D858690F91FBB4A6570FD18AA1CBFBB8C7BED301AAD7C189795C1A6539E83B');
  });
  ...
  it('CasinocoinAPI - implicit server port', function () {
    const api = new CasinocoinAPI({ server: 'wss://s1.casinocoin.org' });
  });
