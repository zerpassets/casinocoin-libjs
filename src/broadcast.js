...
const CasinocoinAPI = require('./api').CasinocoinAPI
...
class CasinocoinAPIBroadcast extends CasinocoinAPI {
    constructor(servers, options) {
        ...
        const apis = servers.map(server => new CasinocoinAPI(
    ...
    getMethodNames() {
        const methodNames = []
        for (const name in CasinocoinAPI.prototype) {
            if (CasinocoinAPI.prototype.hasOwnProperty(name)) {
                if (typeof CasinocoinAPI.prototype[name] === 'function') {
