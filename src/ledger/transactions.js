...
function parseAccountTxTransaction(tx) {
    const _tx = tx.tx_blob ? parseBinaryTransaction(tx) : tx
        // casinocoind uses a different response format for 'account_tx' than 'tx'
...
function getTransactions(address: string, options: TransactionsOptions = {}): Promise < GetTransactionsResponse > {
