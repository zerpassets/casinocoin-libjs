...
function isCSCToCSCPayment(payment: Payment): boolean {
  ...
  return (sourceCurrency === 'CSC' || sourceCurrency === 'drops') &&
         (destinationCurrency === 'CSC' || destinationCurrency === 'drops')
}

function isIOUWithoutCounterparty(amount: Amount): boolean {
  return amount && amount.currency !== 'CSC' && amount.currency !== 'drops'
...
function createMaximalAmount(amount: Amount): Amount {
  const maxCSCValue = '100000000000'
  const maxIOUValue = '9999999999999999e80'
  let maxValue
  if (amount.currency === 'CSC') {
    maxValue = maxCSCValue
  } else if (amount.currency === 'drops') {
    maxValue = cscToDrops(maxCSCValue)
  } else {
...
function createPaymentTransaction(address: string, paymentArgument: Payment
  ...
  const amount =
    (isMinAdjustment(payment.destination) && !isCSCToCSCPayment(payment))
  ...
  if (!isCSCToCSCPayment(payment)) {
    ...
    txJSON.SendMax = toCasinocoindAmount(sourceAmount)

    if (isMinAdjustment(payment.destination)) {
      txJSON.DeliverMin = toCasinocoindAmount(destinationAmount)
    }
