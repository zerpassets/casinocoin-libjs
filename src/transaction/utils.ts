...
function prepareTransaction(txJSON: any, api: CasinocoinAPI,
  ...
  function prepareFee(): Promise<object> {
    ...
    if (instructions.fee !== undefined) {
      ...
      if (fee.greaterThan(api._maxFeeCSC)) {
        const errorMessage = `Fee of ${fee.toString(10)} CSC exceeds ` +
          `max of ${api._maxFeeCSC} CSC. To use this fee, increase ` +
          '`maxFeeCSC` in the CasinocoinAPI constructor.'
    ...
    return api.getFee(cushion).then(fee => {
      return api.connection.getFeeRef().then(feeRef => {
        ...
        const maxFeeCSC = instructions.maxFee ?
          BigNumber.min(api._maxFeeCSC, instructions.maxFee) : api._maxFeeCSC
        const maxFeeDrops = common.cscToDrops(maxFeeCSC)
